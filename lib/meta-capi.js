 ...
 *
 * IMPORTANTE: só deve ser chamado quando existir dado real de
 * correlação com o visitante (fbc/fbp, vindos do cookie do Pixel).
 * Não inventamos correlação quando esses dados não estão presentes.
 */
...
async function sendMetaPurchase({ value, currency = "BRL", eventId, fbc, fbp, clientIp, userAgent, email, phone, externalId }) {
  const pixelId = process.env.META_PIXEL_ID;
  const accessToken = process.env.META_ACCESS_TOKEN;

  if (!pixelId || !accessToken) {
    console.warn("[meta-capi] META_PIXEL_ID/META_ACCESS_TOKEN não configurados — envio pulado.");
