...
function getPayerCpfCnpj() {
  const value = document.getElementById("payer-cpf-cnpj")?.value || "";
...
async function createPix(value, cpfCnpj) {
...
async function handleDonation(value) {
  const cpfCnpj = getPayerCpfCnpj();

  if (!isValidCpfCnpj(cpfCnpj)) {
    setPixStatus("Informe um CPF com 11 dígitos ou CNPJ com 14 dígitos.", true);
    openPixModal();
    return;
  }
  ...
  try {
    const data = await createPix(value, cpfCnpj);
